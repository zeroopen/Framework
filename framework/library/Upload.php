...
class Upload {
	...
	public function upload($files = '') {
		...
		if (empty($this->rootPath)) {
			$this->rootPath = config::system('upload', 'path');
		}
		/* 检测上传根目录 */
		...
		foreach ($files as $key => $file) {
			...
			if ($this->uploader->save($file, $this->replace)) {
				...
				$info[$key] = $file;
				$this->file[$key] = ltrim($this->rootPath.$file['savepath'].$file['savename'], './');
			} else {
				$this->error = $this->uploader->getError();
			}
		}
	...
	public function getUrl() {
		$host = $this->uploader->getUrl()==='' ? '' : 'http://'.$this->uploader->getUrl(); //取得文件所在固定网址
		$url = array();
		foreach ($this->file as $key => $path) {
			$url[$key] = $host . '/' . $path;
		}
		return $url;
	}
	...
	private function setDriver($driver = null, $config = null) {
		$driver = $driver ?: ($this->driver ?: config::system('upload', 'driver'));
		$config = $config ?: ($this->driverConfig ?: (config::system('upload', $driver) ?: array()));
		$class = ucfirst(strtolower($driver));
		require LIB_PATH . 'Upload/' . $class . 'Driver.php';
