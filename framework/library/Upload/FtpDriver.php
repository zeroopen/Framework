...
class Ftp {
	...
	public function __construct($config) {
		$start = strpos($config['host'], '/');
		$config['rootpath'] = ($start === false) ? '' : substr($config['host'], $start);
		$config['host'] = ($start === false) ? $config['host'] : substr($config['host'], 0, $start);
		/* 默认FTP配置 */
	...
	private function login() {
		...
		if ($this->link) {
			if (ftp_login($this->link, $username, $password)) {
				if (ftp_chdir($this->link, $rootpath)) {
					return true;
				} else {
					$this->error = "FTP目录不存在：{$rootpath}";
				}
			} else {
